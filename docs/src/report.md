...
additive within a schema version, a value the backend could not report is omitted rather than written as a
...
Schema 2 (package 0.3) replaces the AMD-only counter keys with `raw_` and `derived_`
summary keys. Other result layouts are unchanged. See [counter migration](hw_counters.md).
...
- [GPUInspector.jl](https://github.com/pc2/GPUInspector.jl) — NVIDIA-only inspection and micro-benchmarks
  ...
  (resource report, instruction mix, cross-compilation) plus the rocprofv3 / Nsight Compute collectors.
...
- [LIKWID.jl](https://github.com/JuliaPerf/LIKWID.jl) — hardware performance counters through
  ...
  counters in-process on NVIDIA, rocprofv3 / Nsight Compute for per-dispatch collections).
