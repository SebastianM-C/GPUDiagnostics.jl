...
| Adding work stops scaling | `kernel_resources` (theoretical occupancy) against sampler `sm_occupancy_busy_mean` (achieved); `power_capped_fraction` and `sm_clock_MHz_busy_median` — a power-bound kernel gains per cycle, not per second; on MI300 `power_violation_fraction` vs `thermal_violation_fraction` (the SMU's own residencies) says which limiter held it, and `xcd_clock_min/max_MHz` whether the dies agree | `resources_mix.md`, `telemetry.md` |
...
| Need per-dispatch hardware counters on AMD or NVIDIA | `hw_counter_command` wraps the process, `hw_counters` parses, `hw_counter_derived` returns per-dispatch columns; `hw_counter_summary` reduces them. Check architecture and measurement denominators. | `hw_counters.md` |
...
- **Counters carry the profiler's units.** AMD `SQ_INSTS_*` are per-wave (× `wave_size` / slots); `*_sum` unit
  counters are event counts (never × wave); SQ wave-cycle counters are quad-cycles on CDNA, plain cycles on RDNA; the dispatch's cycles are
