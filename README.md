...
| What FP64 / FP32 rate can scalar code reach on this device, at the clocks it holds? | `peak_flops_probe` / `measure_peak_flops` (FMA-chain probe swept over chains × launch size, never routed to matrix units; `measure_gemm_flops` is the matrix-unit reference) | [measured peak](https://SebastianM-C.github.io/GPUDiagnostics.jl/dev/peaks_probes/) |
